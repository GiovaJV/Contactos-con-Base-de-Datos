...
const getContacts = async () => {
  const response = await fetch('http://localhost:3003/contactos', { method: 'GET' });
const contact = await response.json();
const userContacts = conctacts.filter(contactos => contactos.username === user.username);
userContacts.forEach(contactos => {
  const li = document.createElement('li');
    li.classList.add('contact');
    li.id = contact.id;
    li.innerHTML = `
...
form.addEventListener('submit', async e => {
  e.preventDefault();
  const responseJSON = await fetch('http://localhost:3003/contactos', { 
    method: 'POST',
    headers: {
        'Content-Type': 'application/json'
    },
    body: JSON.stringify({text: nameInput.value, user: user.username}),
});
...
list.addEventListener('click', e => {
  ...
  list.addEventListener('click', async e => {
    if (e.target.classList.contains('delete-btn')) {
      ...
      await fetch(`http://localhost:3003/contactos/${id}`, {method: 'DELETE'});
    }
...
window.location.href = 'http://127.0.0.1:5500/Index/index.html';
  ...
  if (!user) {
   windown.location.href = 'http://127.0.0.1:5500/Index/index.html';
  }
